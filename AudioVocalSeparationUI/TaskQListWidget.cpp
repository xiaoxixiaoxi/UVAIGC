#include "TaskQListWidget.h"
#include "VideoQtWidgets.h"

TaskQListWidget::TaskQListWidget(QWidget *parent)
    : QListWidget(parent)
{
...
//设置可以多选
    ...
    connect(this, &TaskQListWidget::customContextMenuRequested,
        this, &TaskQListWidget::show_Contextmenu1);
...
TaskQListWidget::~TaskQListWidget()
{

}

void TaskQListWidget::reloadData()
{
...
void TaskQListWidget::addData(std::vector<AVS_Task>* avs_Tasks)
{
    this->avs_Tasks = avs_Tasks;
}

void TaskQListWidget::setSelect_row(qint64 index) {
    this->select_row = index;
}

void TaskQListWidget::del()
{
...
//显示原文件 和输出文件
void TaskQListWidget::showRawPath(int type)
{
    ...
    for (size_t i = 0; i < items.count(); i++)
    {
        ...
        AVS_Task& obj =  this->avs_Tasks->data()[index];
...
void TaskQListWidget::resetObj(int type) {
...
// 导出 0 srt  1为txt
void TaskQListWidget::ExportObj(int type) {
...
void TaskQListWidget::show_Contextmenu1(const QPoint& pos)
{
...
//        cmenu->addAction(out_txt);
        ...
        //connect(delete_step, &QAction::activate, this, &TaskQListWidget::del);
